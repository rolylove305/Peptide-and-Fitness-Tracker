...
function cleanText(value: unknown, maximumLength: number): string {
  const text = value instanceof Error ? value.message : String(value ?? 'Unknown client error');
  return text.replace(/\s+/g, ' ').trim().slice(0, maximumLength) || 'Unknown client error';
}

function stackText(error: unknown, componentStack: string | null | undefined): string | null {
...
function fingerprint(kind: ClientErrorReport['kind'], message: string, route: string): string {
  let hash = 2166136261;
...
function shouldSend(fingerprintValue: string): boolean {
  try {
    ...
    window.sessionStorage.setItem(RECENT_FINGERPRINTS_KEY, JSON.stringify(recent));
    return true;
  } catch {
    return true;
  }
...
async function readServiceWorkerBuildId(): Promise<string | null> {
  ...
  return new Promise((resolve) => {
    ...
    channel.port1.onmessage = (event: MessageEvent<{ buildId?: unknown }>) => {
      ...
      resolve(typeof event.data?.buildId === 'string' ? event.data.buildId.slice(0, 160) : null);
    };
