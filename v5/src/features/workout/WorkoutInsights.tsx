...
function calculateStreaks(sessions: WorkoutSessionSummary[]) {
  const ordinals = [...new Set(sessions.map((session) => dayOrdinal(session.completed_at)))].sort(
    (a, b) => a - b,
  );
  ...
  return (
    <div className="insights-chart-wrap">
      ...
      >
        ...
        {points.map(({ x, y, week }) => (
          <g key={`point-${week.key}`}>
            ...
            <text className="insights-chart-value" x={x} y={Math.max(14, y - 12)} textAnchor="middle">
              {week.sets}
            </text>
            <text className="insights-chart-label" x={x} y={height - 14} textAnchor="middle">
              {week.label}
            </text>
          </g>
        ))}
      ...
      <div className="insights-chart-legend" aria-hidden="true">
        <span><i className="insights-legend-bar" />Workouts</span>
        <span><i className="insights-legend-line" />Working sets</span>
      </div>
    </div>
  );
}
...
export function WorkoutInsights() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const failed = [historyResult, recordResult, muscleResult].find((result) => !result.ok);
      if (failed && !failed.ok) {
  ...
  const weeks = useMemo<WeekBucket[]>(
    () =>
      Array.from({ length: rangeWeeks }, (_, index) => {
        ...
        const unitSessions = weekSessions.filter((session) => session.weight_unit === weightUnit);
        return {
          ...
          duration: weekSessions.reduce((sum, session) => sum + session.duration_seconds, 0),
          volume: unitSessions.reduce((sum, session) => sum + session.total_volume, 0),
        };
      }),
  ...
  const adherence = targetWorkouts > 0 ? Math.round((totals.workouts / targetWorkouts) * 100) : null;
  ...
  const muscles = useMemo(() => {
    ...
    muscleVolume
      ...
      .forEach((entry) => {
        totalsByMuscle.set(
          entry.muscle_group,
          (totalsByMuscle.get(entry.muscle_group) ?? 0) + entry.completed_set_count,
        );
      });
  ...
  const observations = useMemo(() => {
    ...
    const workoutDelta = percentDelta(comparison.current.workouts, comparison.previous.workouts);
    if (workoutDelta === null) notes.push('Training activity began during the most recent comparison period.');
    else if (workoutDelta > 15) notes.push(`Workout frequency increased ${workoutDelta}% in the latest comparison period.`);
    ...
    if (topMuscle && totalMuscleSets > 0) {
      ...
      else notes.push(`Training volume is distributed across ${muscles.length} recorded muscle groups.`);
    }
    return notes.slice(0, 3);
  }, [adherence, comparison, muscles, profile?.daysPerWeek, rangeWeeks, totalMuscleSets, totals.workouts]);

  if (status === 'loading') {
    return (
      <section className="workout-insights workout-insights--loading" aria-live="polite" aria-busy="true">
        <div className="insights-skeleton insights-skeleton--hero" />
        <div className="insights-kpi-grid">
          {Array.from({ length: 4 }, (_, index) => <div className="insights-skeleton insights-skeleton--card" key={index} />)}
        </div>
  ...
  if (status === 'error') {
    return (
      <section className="workout-insights-state" role="alert">
        ...
        <p>{error}</p>
        <button className="secondary-button" type="button" onClick={() => setRequestVersion((value) => value + 1)}>Try again</button>
      </section>
    );
  }
  ...
  if (status === 'empty') {
    return (
      <section className="workout-insights-state">
        ...
        <p>Completed sessions will unlock weekly trends, muscle balance, records and consistency patterns.</p>
      </section>
    );
  }

  return (
    <section className="workout-insights" aria-labelledby="workout-insights-heading">
      <header className="workout-insights-heading">
        <div>
          <p className="eyebrow">Training analytics</p>
          <h2 id="workout-insights-heading">Understand what your training is actually doing</h2>
          <p>Review frequency, workload, consistency, muscle balance and current records using completed workout data.</p>
        </div>
        <button className="overview-refresh-button" type="button" onClick={() => setRequestVersion((value) => value + 1)}>Refresh</button>
      </header>
      ...
      <div className="insights-control-row" aria-label="Analytics filters">
        <div className="insights-segmented-control" aria-label="Time range">
          {rangeOptions.map((option) => (
            <button
              type="button"
              className={rangeWeeks === option ? 'insights-segment insights-segment--active' : 'insights-segment'}
        ...
        {availableUnits.length > 1 ? (
          <div className="insights-segmented-control" aria-label="Weight unit">
            {availableUnits.map((unit) => (
              <button
                type="button"
                className={weightUnit === unit ? 'insights-segment insights-segment--active' : 'insights-segment'}
      ...
      <div className="insights-kpi-grid" aria-label={`${rangeWeeks}-week totals`}>
        <article><span>Workouts</span><strong>{totals.workouts}</strong><small>{(totals.workouts / rangeWeeks).toFixed(1)} per week</small></article>
        <article><span>Working sets</span><strong>{formatNumber(totals.sets)}</strong><small>{formatNumber(totals.reps)} total reps</small></article>
        <article><span>Training time</span><strong>{formatDuration(totals.duration)}</strong><small>{formatDuration(averageDuration)} average</small></article>
        <article><span>Training volume</span><strong>{formatCompact(totals.volume)}</strong><small>{weightUnit} · matching-unit sessions</small></article>
        <article><span>Current streak</span><strong>{streaks.current}</strong><small>Longest: {streaks.longest} days</small></article>
        <article><span>Plan completion</span><strong>{adherence === null ? '—' : `${adherence}%`}</strong><small>{targetWorkouts > 0 ? `${totals.workouts} of ${targetWorkouts} target sessions` : 'Complete your profile to set a target'}</small></article>
      </div>

      <article className="insights-panel insights-panel--chart">
        <header className="insights-panel-heading">
          <div><span className="overview-kicker">Weekly trend</span><h3>Frequency and working sets</h3></div>
          {bestWeek ? <span>Best week: {bestWeek.label} · {bestWeek.workouts} workouts</span> : null}
        </header>
      ...
      <div className="insights-two-column">
        <article className="insights-panel">
          <header className="insights-panel-heading">
            <div><span className="overview-kicker">Consistency map</span><h3>{rangeWeeks} weeks of activity</h3></div>
            <span>{filteredSessions.length} sessions</span>
          </header>
          <div className="insights-weekday-labels" aria-hidden="true">
            {weekdayLabels.map((label) => <span key={label}>{label}</span>)}
          </div>
          <div className="insights-heatmap" aria-label="Completed workout activity by day">
            {heatmapDays.map((day) => {
              const label = new Intl.DateTimeFormat(undefined, { weekday: 'long', month: 'short', day: 'numeric' }).format(day.date);
              return (
                <span
        ...
        <article className="insights-panel insights-panel--comparison">
          <header className="insights-panel-heading">
            <div><span className="overview-kicker">Momentum</span><h3>Latest {comparison.label}</h3></div>
          </header>
          <div className="insights-comparison-list">
            <div><span>Workouts</span><strong>{comparison.current.workouts}</strong><em>{deltaLabel(comparison.current.workouts, comparison.previous.workouts)}</em></div>
            <div><span>Working sets</span><strong>{comparison.current.sets}</strong><em>{deltaLabel(comparison.current.sets, comparison.previous.sets)}</em></div>
            <div><span>Training time</span><strong>{formatDuration(comparison.current.duration)}</strong><em>{deltaLabel(comparison.current.duration, comparison.previous.duration)}</em></div>
            <div><span>Volume ({weightUnit})</span><strong>{formatCompact(comparison.current.volume)}</strong><em>{deltaLabel(comparison.current.volume, comparison.previous.volume)}</em></div>
          </div>
        </article>
      </div>
      ...
      <div className="insights-two-column insights-two-column--lower">
        <article className="insights-panel">
          <header className="insights-panel-heading">
            <div><span className="overview-kicker">Muscle balance</span><h3>Completed sets by muscle group</h3></div>
            ...
            <div className="insights-muscle-list">
              {muscles.slice(0, 8).map((muscle) => (
                <div className="insights-muscle-row" key={muscle.name}>
                  <div><span>{muscle.name}</span><strong>{muscle.sets} sets</strong></div>
                  <div className="insights-muscle-track"><span style={{ width: `${Math.max(6, (muscle.sets / maximumMuscleSets) * 100)}%` }} /></div>
                  <small>{totalMuscleSets > 0 ? Math.round((muscle.sets / totalMuscleSets) * 100) : 0}%</small>
                </div>
              ))}
            </div>
          ) : <p>Muscle-group analytics will appear after completed working sets are recorded.</p>}
        </article>

        <article className="insights-panel">
          <header className="insights-panel-heading">
            <div><span className="overview-kicker">Current records</span><h3>Strongest recorded performances</h3></div>
            ...
            <div className="insights-record-list">
              {selectedRecords.map((record) => (
                <div key={`${record.exercise_id}:${record.weight_unit}`}>
                  <div><strong>{record.exercise_name}</strong><span>{record.primary_muscle_group}</span></div>
                  <dl>
                    <div><dt>Top load</dt><dd>{record.heaviest_weight === null ? '—' : `${formatNumber(record.heaviest_weight, 1)} ${record.weight_unit}`}</dd></div>
                    <div><dt>Most reps</dt><dd>{record.highest_reps ?? '—'}</dd></div>
                    <div><dt>Best set</dt><dd>{record.best_set_volume === null ? '—' : formatCompact(record.best_set_volume)}</dd></div>
                  </dl>
                </div>
              ))}
            </div>
      ...
      <aside className="insights-observation-panel">
        <div><span aria-hidden="true">AI</span><div><p className="eyebrow">Evidence summary</p><h3>What the data says</h3></div></div>
        <ul>{observations.map((observation) => <li key={observation}>{observation}</li>)}</ul>
        <p>These observations describe logged patterns only. They do not automatically change your routine or replace recovery judgment.</p>
      </aside>
    </section>
  );
}
