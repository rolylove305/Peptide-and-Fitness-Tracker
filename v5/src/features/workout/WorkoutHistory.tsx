...
function formatVolumeTotals(history: WorkoutSessionSummary[]): string {
  ...
  history.forEach((session) => {
    totals.set(
      session.weight_unit,
      (totals.get(session.weight_unit) ?? 0) + session.total_volume,
    );
  });
...
function formatSet(workoutSet: WorkoutHistorySet): string {
  ...
  const effort = workoutSet.rpe
    ? ` · RPE ${formatNumber(workoutSet.rpe, 1)}`
    : '';
  if (weight === null)
    return `Set ${workoutSet.set_number}: ${reps} reps${effort}`;
    ...
    <div className="history-session-detail">
      {detail.exercises.map((exercise) => (
        <article className="history-exercise-detail" key={exercise.id}>
              ...
              <span
                className={
                  workoutSet.is_completed
                    ? 'history-set-chip'
                    : 'history-set-chip history-set-chip--skipped'
                }
                key={workoutSet.id}
              >
          ...
          {exercise.notes ? (
            <p className="history-exercise-notes">{exercise.notes}</p>
          ) : null}
        </article>
      ))}
    </div>
...
export function WorkoutHistory() {
  ...
  const overview = useMemo(
    () => ({
      ...
      sets: historyState.history.reduce(
        (total, session) => total + session.working_set_count,
        0,
      ),
      reps: historyState.history.reduce(
        (total, session) => total + session.total_reps,
        0,
      ),
  ...
  const maxMuscleVolume = Math.max(
    1,
    ...muscleProgress.map((item) => item.volume),
  );
      ...
      [...historyState.records]
        .sort((a, b) => {
          const weightDifference =
            (b.heaviest_weight ?? -1) - (a.heaviest_weight ?? -1);
  ...
  if (historyState.status === 'empty') {
    return (
      ...
      >
        ...
        <p>
          Finish a workout in the Train tab. BioTrack will automatically create
          your history, records and muscle-group progress from the sets you
          save.
        </p>
      </section>
    );
    ...
    >
      <div className="section-heading history-heading">
        <div>
          ...
          <p>
            Review completed sessions, previous performance, personal records
            and 90-day muscle volume.
          </p>
        </div>
        <button
      ...
      <div className="history-overview-grid">
        ...
        <article>
          ...
          <strong>{formatNumber(overview.sets)}</strong>
        </article>
        <article>
          <span>Total reps</span>
          <strong>{formatNumber(overview.reps)}</strong>
        </article>
        <article>
          <span>Training volume</span>
          <strong>{overview.volume}</strong>
        </article>
      </div>
      ...
      <div className="history-dashboard-grid">
            ...
            {historyState.history.map((session) => {
              ...
              return (
                <article
                  className={
                    selected
                      ? 'history-session-card history-session-card--open'
                ...
                >
                  ...
                  >
                    ...
                    <div className="history-session-stats">
                      ...
                      <span>
                        {formatNumber(session.total_volume)}{' '}
                        {session.weight_unit}
                      </span>
                    </div>
                  </button>
                  {selected && loading ? (
                    <p className="history-detail-loading">
                      Loading set details…
                    </p>
                  ) : null}
        ...
        <section
          className="history-panel"
          aria-labelledby="muscle-progress-heading"
        >
          <div className="history-panel-heading">
            <div>
              ...
              {muscleProgress.map((item) => (
                <article key={`${item.muscleGroup}:${item.weightUnit}`}>
                  <div className="muscle-progress-label">
                    ...
                    <span>
                      {formatNumber(item.volume)} {item.weightUnit}
                    </span>
                  </div>
                  <div className="muscle-progress-track" aria-hidden="true">
                    <span
                      style={{
                        width: `${Math.max(4, (item.volume / maxMuscleVolume) * 100)}%`,
                      }}
                    />
                  </div>
                </article>
          ...
          {topRecords.map((record) => (
            ...
            >
              ...
              <div className="exercise-record-values">
                ...
                <div>
                  <span>Most reps</span>
                  <strong>{record.highest_reps ?? '—'}</strong>
                </div>
                <div>
                  ...
                  <strong>
                    {record.best_set_volume === null
                      ? '—'
                      : `${formatNumber(record.best_set_volume)} ${record.weight_unit}`}
                  </strong>
                </div>
              </div>
              <footer>
                {record.session_count} sessions · Last{' '}
                {formatDate(record.last_performed_at)}
              </footer>
            </article>
          ))}
        </div>
